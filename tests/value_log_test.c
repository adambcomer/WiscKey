...
TestValueLog_append()
{
  ...
  int res = ValueLog_append(
    log, &pos, key1, strlen(key1) + 1, value1, strlen(value1) + 1);
  assert(res == 0);
  ...
  res = ValueLog_append(
    log, &pos, key2, strlen(key2) + 1, value2, strlen(value2) + 1);
  assert(res == 0);
...
TestValueLog_get()
{
  ...
  res = ValueLog_append(log,
                        &pos2,
                        "lime",
                        strlen("lime") + 1,
                        "Key Lime Pie",
                        strlen("Key Lime Pie") + 1);
  assert(res == 0);
