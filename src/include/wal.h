 ...
 *
 * The WAL holds a running log of the operations that were applied to the MemTable. When the database restarts, the WAL
 * is replayed to recover the MemTable.
 */
struct WAL {
 ...
 * @return This function returns 0 if the WAL successfully replayed and -1 if there was an error.
 */
 ...
 *
 * For sets, the `value_loc` should be the position in the ValueLog that the value entry was written to. For deletes,
 * `value_loc` should be -1 to indicate a tombstone.
 *
 * @param wal The WAL to append a MemTable operation to.
 * @param key The key to apply an operation to.
 * @param key_len The length of the key.
 * @param value_loc The location in the ValueLog of the value or -1 if it is being deleted.
 * @return This function returns 0 if the operation was successfully writen to the WAL and -1 if there was an error.
 */
 ...
 *
 * This function forcefully flushes the changes to the WAL to disk. Use this function sparingly as is will heavily
 * reduce the write throughput of the WAL.
 *
 * @param wal The WAL to flush to disk.
 * @return This function returns 0 if the WAL successfully synced to the disk and -1 if there was an error.
 */
 ...
 * @brief Frees the WAL.
 *
 * Note: This function won't delete the file or the underlying data in the WAL. This function only frees the memory
 * allocated to a WAL.
 *
